...
class HRNet(nn.Module):
    ...
    def __init__(self,
        ...
        ##----------------------------
        self.hm_size = [2, 2]
        self.hm_size = self.extra['data_cfg']['heatmap_size'] ##----------------------------
        ...
        self.norm3_name, norm3 = build_norm_layer(self.norm_cfg, 64, postfix=2)
        self.norm4_name, norm4 = build_norm_layer(self.norm_cfg, 64, postfix=2)
    ...
    @property
    def norm4(self):
        """nn.Module: the normalization layer named "norm4" """
    ...
    def _freeze_stages(self):
        ...
        if self.frozen_stages >= 0:
            ...
            for m in [self.conv1, self.norm1, self.conv2, self.norm2, self.conv3, self.norm3, self.conv4, self.norm4]:
    ...
    # # #-------------2023-04-23------------对于8以下的重新定义卷积（因为hr没有？
    def forward(self, x):
        if self.hm_size[0] == 64:
            return self.forward_64_32_16(x)
        elif self.hm_size[0] == 32:
            return self.forward_64_32_16(x)
        if self.hm_size[0] == 16:
            return self.forward_64_32_16(x)
        if self.hm_size[0] == 8:
            return self.forward_8(x)
        if self.hm_size[0] == 4:
            return self.forward_4(x)
