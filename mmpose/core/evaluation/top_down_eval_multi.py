...
##------------------------------------------
def block_search(hm, center, H, W, step_h=2, step_w=3):
    # import pdb
    # pdb.set_trace()
    ##search window of range [-2, +2] is centered around center
    ## 
    ...
    sub_prob = hm[Ymin:Ymin+step_h, Xmin:Xmin+step_w]  #这里为啥不用Ymax虽然一般不会超界？
    sub_anchor=[]
    ...
    return sub_prob, sub_anchor  #返回hm和anchor 内容后面再看
...
def decode_dis(prob, sigma):
    ...
    for p0 in prob:
        for p in p0:
            p = np.clip(p, 0.000000001, 1.0) #将一个nd.array的值限制在给定的上下界
            dis_decode = np.sqrt( -2. * sigma * sigma * np.log(p) ) #sigma=2 貌似可以直接对矩阵求？后面看下。
        ...
        if post_process == 'unbiased':  # alleviate biased coordinate
            ...
            for n in range(N):
                for k in range(K):
                    ...
                    ##------------------------------------------ Anchor Sampling  
                    sub_prob, sub_anchor = block_search(heatmap, preds[n][k], H, W, step_h=2, step_w=3 )
                    sub_dis = decode_dis(sub_prob, sigma=2)
